// 7. Calculadora de Juros Compostos 2:
// Refaça o exercício anterior, só que agora use juros mensais serão aleatórios entre 0,5% e 5% ao mês.
// O usuário deve digitar o valor inicial e o tempo do investimento. Ao final informar o valor total.
...
int main() { // Função principal do programa
    ...
    while (repetir == 's' || repetir == 'S') // Enquanto o valor de 'repetir' for 's' ou 'S', o loop continuará
    {
        ...
        for (i = 0; i < meses; i++) { // Loop que se repete 'meses' vezes
            taxa = (rand() % 46 + 5) / 10.0; // Gera uma taxa de juros aleatória entre 0.5% e 5%
